...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var paymentTermsData *dpfm_api_output_formatter.PaymentTerms
	for _, a := range accepter {
		switch a {
		case "PaymentTerms":
			h := c.paymentTermsDelete(input, output, log)
			paymentTermsData = h
			if h == nil {
				continue
			}
...
func (c *DPFMAPICaller) paymentTermsDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.PaymentTerms {
	sessionID := input.RuntimeSessionID
	paymentTerms := c.PaymentTermsRead(input, log)
	paymentTerms.BaseDate = input.PaymentTerms.BaseDate
	paymentTerms.IsMarkedForDeletion = input.PaymentTerms.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": paymentTerms, "function": "PaymentTermsPaymentTerms", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "PaymentTerms Data cannot delete"
		return nil
	}
